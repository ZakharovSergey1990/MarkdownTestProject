...
public class InlineParserCustom implements InlineParser {
    ...
    private static final Pattern PROJECT = Pattern.compile("\\b(uc-project)://[-a-zA-Z0-9+&@#/%?=~_|!:,.;]*[-a-zA-Z0-9+&@#/%=~_|]");

    private static final Pattern HTTP = Pattern.compile("\\b(https?|http)://[-a-zA-Z0-9+&@#/%?=~_|!:,.;]*[-a-zA-Z0-9+&@#/%=~_|]");

    private static final Pattern MENTION = Pattern.compile("\\B@[\\w\\-]+");
    ...
    public InlineParserCustom(InlineParserContext inlineParserContext) {
    ...
    public void parse(String content, Node block) {
        reset(content.trim());
        Log.d("Markdown", "parse() "+ block.toString());
        Node previous = null;
        while (true) {
    ...
    private Node parseInline(Node previous) {
        ...
        Node node;
        Log.d("Markdown", "parseInline c= "+ c);
        switch (c) {
            case '\n':
    ...
    private Node parseProject() {
        Log.d("Markdown", "parseProject");
        String m = match(PROJECT);
        Log.d("Markdown", "parseProject m = " + m);
        if (m != null) {
            ProjectNode node = new ProjectNode(m);
            return node;
        } else {
            return null;
        }
    ...
    private Node parseHttp() {
        Log.d("Markdown", "parseHttp");
        String m = match(HTTP);
        Log.d("Markdown", "parseHttp m = " + m);
        if (m != null) {
            HttpNode node = new HttpNode(m);
            return node;
        } else {
            return null;
        }
    ...
    private Node parseMention() {
        Log.d("Markdown", "parseMention");
        String m = match(MENTION);
        Log.d("Markdown", "parseMention m = " + m);
        if (m != null) {
            MentionNode node = new MentionNode(m);
            return node;
        } else {
            return null;
        }
